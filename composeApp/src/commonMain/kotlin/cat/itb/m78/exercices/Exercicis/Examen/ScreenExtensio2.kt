...
@Composable
fun ScreenStudentListExtensioVM(navigateToStudentList2:() -> Unit) {
    val model = viewModel { Extensio2ViewModel() }
    ScreenStudentExtensioList(model.studentsList.value,model::searchStudent, model::insert,  model::count)
}

@Composable
fun ScreenStudentExtensioList(students: List<Students>, searchStudent:(String) -> Unit, insert:(Int, String) -> Unit, count:(Int) -> Unit) {
            ...
            LazyColumn(modifier = Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(15.dp)) {
                ...
                itemsIndexed(filteredList) { _, student ->
                        ...
                        }))
                    {
                        ...
                        Text("Total Faltes: " + count(student.idStudent).toString())
                    }
                }
            }
        }
